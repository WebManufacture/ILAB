...
Frame._initFrame = function () {
    ...
    try {
        if (Frame.nodePath.indexOf("http://") == 0 || Frame.nodePath.indexOf("https://") == 0) {
            ...
            console.log("Requiring from frame",Frame.nodePath);
            var node = require(Frame.nodePath);
            console.log("Starting frame",Frame.node);
...
Frame._startFrame = function (node, params) {
    return new Promise(resolve => {
        try{
            params = params ? params : getEnvParam("params", {});
...
Frame.startChild = function(params){
    ...
    cp.on("message", (obj) => {
        if (typeof obj == "object"){
            if (obj.type == "error"){
                if (obj.item) {
                    return process.emit("child-error", new Error(obj.message), obj.item);
                } else {
...
if (Frame.isChild) {
    ...
    ServicesManager = require(Path.resolve("./Services/ServicesManager.js"));
    runNode(ServicesManager, "ServicesManager");
    ServiceProxy = useSystem("ServiceProxy");
    Frame.rootId = "ServicesManager";
    setTimeout(()=>{
        ServiceProxy.init().then(function (servicesManager) {
            ServicesManager.GetServices = ServiceProxy.GetServices;
            ServicesManager.GetServicesInfo = ServiceProxy.instance.GetServicesInfo;
            ServicesManager.GetService = ServiceProxy.GetService;
            runNode(proxyService,  "ServicesWebSocketProxy").then((service)=>{
                proxyService = service;
                proxyService.knownServices = {
                    "ServicesManager": Frame.getPipe("ServicesManager"),
                };
            });
        });
    }, 50);
