...
Inherit = function (Child, Parent, mixin)
{
	...
	if (typeof Parent == 'function') {
		...
		Child.super = Child.super_ = parentConstructor;
		childProto._base = Parent.prototype;
		childProto._super = childProto.super = function(){
		    this._super = this.super = this._base ? this._base._super : undefined;
		    parentConstructor.apply(this, arguments);
		    this._super = this.super = parentConstructor;
		};
		return Child;
	}
...
Date.currentTime = function(withMilliseconds){
	const date = new Date();
	if (withMilliseconds){
		return gfdp(date.getHours()) + ":" + gfdp(date.getMinutes()) + ":" + gfdp(date.getSeconds()) + "." + date.getMilliseconds();
	}
	else{
		return gfdp(date.getHours()) + ":" + gfdp(date.getMinutes()) + ":" + gfdp(date.getSeconds());
	}
};
...
Selector.prototype = {
    ...
    is : function(obj){
        ...
        if (objType == "object"){
            ...
            if (objType == "Object") objType = "object";
        }
        if (objType == "function"){
        ...
        if (this.type && objType != '*' && this.type != "*" && this.type != objType) return false;
        if (this.type && this.type != "*") matches += objType;
        if (this.id) matches += "#" +this.id;
        if (this.classes){
            if (obj.tags){
                var tags = " " + obj.tags + " ";
                for (var i = 0; i < this.classes.length; i++){
        ...
        if (this.conditions)
            for (var i = 0; i < this.conditions.length; i++){
                ...
                if (c.condition){
                    ...
                    matches += c.condition + c.value;
                }
            }
        return matches == "" ? true : matches;
    },
...
Selector.Navigator = function(structure) {
    this.root = structure ? structure : [];
    this.initStructure();
    this.timeout = 5000;
}

Inherit(Selector.Navigator, {
...
(function(){
    for (var item in __EventEmitterPrototype){
    	EventEmitter.prototype[item] = __EventEmitterPrototype[item];
    }
})();
function SystemTasksScheduler(){

}

SystemTasksScheduler.tasks = [];

Inherit(SystemTasksScheduler, EventEmitter, {
    createTask : function(taskId, task){

    },

    timeout: function(time, task){
    ...
    process: function(task, dependencies){
...
function SystemTask(handler){
    this.handler = handler;
}

Inherit(SystemTask, EventEmitter, {
    timeout: function(time, task){
    ...
    _call: function(){
        if (this.handler){
            ...
            if (typeof(result) == "object"){
                if (result instanceof SystemTask){
                    this.next(result);
                    return;
                }
            }
...
if ((window.WS == undefined || window.WS == null) && (window.DOM == undefined || window.DOM == null || window.DOM.JaspVersion < 3.15)) {
	...
	DOM.__internalEE = new EventEmitter();
	...
	DOM._on = DOM.On = DOM.on = function(event, handler, bubble){
	    if (this == DOM){
    		if (event == "load"){
    		    ...
    		    DOM.__internalEE.on(event, handler);
    		}
	    } else {
	...
	DOM._do = DOM.Do = DOM.do = function(event){
	    if (this == DOM){
    		return DOM.__internalEE.do(event);
	    } else {
...
return wrapper.firstChild;*/
	...
	DOM.GetContext = DOM.getContext = function(){
        if (this._domContext) {
            return this._domContext;
        }
        if (this.parentElement){
            return this.parentElement.getContext();
        }
        return null;
    };

    DOM.SetContext = DOM.setContext = function(value){
        this._domContext = value;
        return value;
    };
	...
	DOM.Add = DOM.add = DOM._add = function(entity, value, param) {
		...
		if (typeof (entity) == "string") {
			...
			if (WS.CallEvent(elem, "onAddElem", entity)) {
                ...
                elem.add(template.content, value);
			}
			return elem.lastChild;
		}
        if (entity instanceof DocumentFragment) {
            while(entity.childNodes.length) {
                var last = elem._add(entity.childNodes[0], value);
            }
            return last;
        }
		if (entity instanceof Node) {
			if (WS.CallEvent(elem, "onAddElem", entity)) {
				entity._creationContext = value;
		...
		if (entity.length || entity.IsElementArray) {
			for (var j = 0; j < entity.length; j++) {
				var last = elem._add(entity[j], value);
			}
			return last;
		}
		if ((entity) instanceof Object) {
...
return element;
	...
	WS.ExtendElement = function(proto) {
		...
		Object.defineProperty(proto,"selector",{
		    get: function(){
				return this.tagName.toLowerCase() + (this.id ? "#" + this.id : "") + (this.className ? "." + this.className.replace(/\s/ig, '.') : "");
			}
		});
		Object.defineProperty(proto, "path",{
		    get: function(){
		        if (this.parentNode && this.tagName != 'BODY' && this.parentNode.path){
		            return this.parentNode.path + "/" + this.selector;
		        } else {
...
if (!UsingDOM("KLabNet")) (function(){
	...
	function _NetInternal(url, isPermanent){
		if (!url){
	...
	_NetInternal.prototype.Gdd = _NetInternal.prototype.GET = _NetInternal.prototype.get;
	_NetInternal.prototype.Add = _NetInternal.prototype.POST = _NetInternal.prototype.add;
	_NetInternal.prototype.All = _NetInternal.prototype.SEARCH = _NetInternal.prototype.browse = _NetInternal.prototype.all;
	_NetInternal.prototype.Set = _NetInternal.prototype.PUT = _NetInternal.prototype.set;
	_NetInternal.prototype.Del = _NetInternal.prototype.DELETE = _NetInternal.prototype.del;

	NET = Net = new _NetInternal();
...
WS.Header._add(scripts[i]);
	...
	M.LoadScriptInternal = function(ourl, module, cmod, cache, isDeffered) {
		var script = M.CreateScript(ourl, cache);
		script.cls("created");
		if (module != null) {
		    if (isDeffered){
		        M.OnModuleRegistered.subscribe(function(){
    		        module._add(script);
...
data = mod.html();
	...
	M.ParsingUrlScriptContext.Process = M.ProcessUrlScript = function(script, context, module, from) {
		...
		if (url != null) {
			...
			M.LoadScriptInternal(url, module, from, null, script._is(".deffered"));
		}
		return true;
	};
...
if (!UsingDOM("Contexts")) {
	...
	C.ProcessNode = function(elem, context, parentElement, param1){
		...
		var result = context.Process(elem, context, parentElement, param1);
		if (result) {
			processed++;
