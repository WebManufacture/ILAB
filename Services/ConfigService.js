...
function ConfigService(params) {
    ...
    var configPath = this.configPath = params.configPath ? params.configPath : (Frame.configFilePath ? Frame.configFilePath :"config.json");
        ...
        if (filesService) {
            ...
            if (params.watchConfigFile) {
                filesService.Watch(Path.resolve(self.configPath), false).then((fpath) => {
                    ...
                    console.log("Watching " + self.configPath);
                }).catch((err) => {
                    console.log("Can't watch " + self.configPath);
                    console.log(err);
                });
            }
...
Inherit(ConfigService, Service, {
    ...
    watchServiceFileChange(serviceId, options){
        ...
        return self.filesService.Watch(path, false).then((fpath) => {
            var changing = false;
            self.filesService.on("watch:" + path, (change, path) => {
                if ((change == "change" || change == "rename") && !changing) {
