...
function ForkMon(socketPort){
	...
	this.cmd = function(){
	    if (os.platform() == "linux"){
	        var cp =  require('child_process').spawn('node', ["-i"], {shell: true, stdio: ['pipe', 'pipe', 'inherit']});
	...
	this.start = function(){
		...
		console.warn("\n>>> Node socket created on " + this.port);
		...
		this.SockServer.on('connection', function (socket) {
			...
			console.warn("\n>>> Node socket connection: " + path);
			socket.on('error', function (err) {
				console.error("\n>>> Node socket server error: " + path);
				console.error(err);
			});
...
util.inherits(ForkMon, EventEmitter);
